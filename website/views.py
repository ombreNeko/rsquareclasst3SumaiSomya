...
@login_required
def student_guardian_list_temp(request, pk):
    student = get_object_or_404(models.RsquareUser, pk=pk, role='STUDENT')
    context= None
    if not (request.user.is_owner() or request.user.is_branch_manager()):
        return redirect('dashboard')
    ...
    if request.method == 'POST':
        ...
        # if student.profile.student_guardian is not None:
        guardian_list = models.Guardian.objects.filter(student = student.profile)
        context = {
            'guardian_list' : guardian_list
        }

        return render(request, 'student/test3/guardian_form2.html', context)
